...
ALTER TABLE campaign_activities ADD COLUMN IF NOT EXISTS attachments JSONB NOT NULL DEFAULT '[]'::jsonb;
...
-- Multiplicative / section scoring policy (editable from /admin/scoring)
ALTER TABLE campaign_settings
...
ALTER TABLE billboards ADD COLUMN IF NOT EXISTS raw_score DOUBLE PRECISION;

ALTER TABLE posters ADD COLUMN IF NOT EXISTS content_hash TEXT;
ALTER TABLE posters ADD COLUMN IF NOT EXISTS raw_score DOUBLE PRECISION;
ALTER TABLE videos ADD COLUMN IF NOT EXISTS content_hash TEXT;
ALTER TABLE videos ADD COLUMN IF NOT EXISTS raw_score DOUBLE PRECISION;
ALTER TABLE social_media_posts ADD COLUMN IF NOT EXISTS raw_score DOUBLE PRECISION;
...
ALTER TABLE videos ADD COLUMN IF NOT EXISTS video_content_type TEXT;
...
ALTER TABLE social_media_posts ADD COLUMN IF NOT EXISTS media_scope TEXT;
...
ALTER TABLE campaign_activities ADD COLUMN IF NOT EXISTS media_scope TEXT;
...
-- Content messages: admin/کارفرما notes on a content card for the owner to read
...
CREATE TABLE IF NOT EXISTS sms_send_reports (
  ...
  manual_score DOUBLE PRECISION DEFAULT 0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
